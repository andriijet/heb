jQuery(document).ready(function ($) {
    ...
    var HI = {
        ...
        S: {
            ...
            "wrapper": {"overflow": "hidden", "background": "rgba(255, 255, 255, 0)"},
            "image": {"enable": true, "slide": 0, "scale": false, "push": false, "scale_percents": {"start": "100", "stop": "110"}, "rotate": false, "rotate_grad_x": {"start": "0", "stop": "0"}, "rotate_grad_y": {"start": "0", "stop": "0"}, "rotate_grad_z": {"start": "0", "stop": "0"}, "perspective": "400", "origin": "center center", "radius_units": "pc", "radius": {"start": "0", "stop": "0"}},
            "overlay": {"enable": true, "background": "rgba(0, 0, 0, 0.6)", "opacity": "1", "fade": true, "slide": false, "slide_direction": "0", "scale": false, "scale_percents": {"start": "0", "stop": "100"}, "rotate": false, "rotate_grad_x": {"start": "0", "stop": "0"}, "rotate_grad_y": {"start": "0", "stop": "0"}, "rotate_grad_z": {"start": "0", "stop": "0"}, "perspective": "400", "origin": "center center"},
            "buttons": {
                "enable": true,
                "set": {"link": true, "view": true},
                "link_single": false,
                "pphoto": true,
                "type": "background",
                "background": "#ffffff",
                "color": "#000000",
                "radius": {"start": "50", "stop": "0"},
                "size": 28,
                "opacity": "1",
                "fade": true,
                "slide": false,
                "slide_direction": "0",
                "scale": false,
                "scale_percents": {"start": "0", "stop": "100"},
                "rotate": false,
                "rotate_grad_x": {"start": "0", "stop": "0"},
                "rotate_grad_y": {"start": "0", "stop": "0"},
                "rotate_grad_z": {"start": "0", "stop": "0"},
                "perspective": "400",
                "origin": "center center"
            },
            "content": {
                "enable": false,
                "enable_parts": {"title": true, "excerpt": true, "categories": true, "shares": true},
                "always": false,
                "shares_enable": {"twitter": true, "facebook": true, "googleplus": true, "pinterest": true, "tumblr": false, "linkedin": false, "reddit": false, "digg": false},
                "title_type": "link",
                "typo": {"title": {"size": 18, "height": 22, "color": "#ffffff", "bold": "bold", "italic": false, "underline": false, "uppercase": false}, "excerpt": {"length": 4, "size": 12, "height": 16, "color": "#ffffff", "bold": false, "italic": false, "underline": false, "uppercase": false}, "categories": {"size": 12, "height": 16, "color": "#ffffff", "bold": false, "italic": false, "underline": 0, "uppercase": false}, "shares": {"size": 14, "color": "#ffffff", "radius": 0}},
        ...
        _radius: function () {
            ...
            HI.C.I.radius.on('change.hi', function () {
                ...
                HI.W.image.css('borderRadius', HI.S.image.radius.start + HI._pc_to_pc(HI.S.image.radius_units));
                HI.W.overlay.css('borderRadius', HI.S.image.radius.start + HI._pc_to_pc(HI.S.image.radius_units));
                HI.W.content_div.css('borderRadius', HI.S.image.radius.start + HI._pc_to_pc(HI.S.image.radius_units));
            }).trigger('change');
        ...
        _set_image_radius_hover: function () {
            HI.W.preview.hover(function () {
                HI.W.image.css('borderRadius', HI.S.image.radius.stop + HI._pc_to_pc(HI.S.image.radius_units));
                HI.W.overlay.css('borderRadius', HI.S.image.radius.stop + HI._pc_to_pc(HI.S.image.radius_units));
                HI.W.content_div.css('borderRadius', HI.S.image.radius.stop + HI._pc_to_pc(HI.S.image.radius_units));
            }, function () {
                HI.W.image.css('borderRadius', HI.S.image.radius.start + HI._pc_to_pc(HI.S.image.radius_units));
                HI.W.overlay.css('borderRadius', HI.S.image.radius.start + HI._pc_to_pc(HI.S.image.radius_units));
                HI.W.content_div.css('borderRadius', HI.S.image.radius.start + HI._pc_to_pc(HI.S.image.radius_units));
        ...
        _set_image_prop: function (data) {
            ...
            HI.C.I.radius_units.prop('checked', false).parent().removeClass('active');
            HI.C.I.radius_units.filter('[value="' + data.radius_units + '"]').prop('checked', true).trigger('change').parent().addClass('active');
        },
        ...
        _save: function () {
            HI.C.save_btn.on('click', function (e) {
                ...
                var tplId = $('#hi-tpl-id').val();
                var tplName = $('#hi-tpl-name').val();
                var tplData = JSON.stringify(HI.S);
                ...
                $.post(ajaxurl, post, function (r) {
                    ...
                    if (resp.id) {
                        ...
                        // check if exist
                        var assign_options = $('select.hi-tpl-assign').find('option[value="' + resp.id + '"]');
                        if (assign_options.length) {
                            assign_options.each(function (i, e) {
                                var assign_option = $(e);
                                assign_option.text(tplName + ' ( id: ' + resp.id + ' )');
                            });
                        }
                        else {
                    ...
                    if (tplId) {
                        hi_refresh_tpl_name(tplId, tplName);
                    }
