...
function hi_tinymce_enqueue( $options ) {
    if ( $options['tinymce'] ) {
        wp_enqueue_script( 'hi_tinymce', HI_URL . 'functions/extension/tinymce/assets/heb-tinymce.js', array( 'jquery' ), HI_VERSION, true );
        wp_enqueue_style( 'hi_tinymce',  HI_URL . 'functions/extension/tinymce/assets/heb-tinymce.css', array(), HI_VERSION );
    }
}
...
function hi_tinymce_template(){
    include HI_PATH . '/functions/extension/tinymce/tpl.php';
}
...
function hi_tinymce_shortcode_init( $content )
{
    ...
    //preg_match_all ("/[^\[caption .*\]](<img .* data-hi_style=.*>\s?)/im", $content, $image_array);

	// Get Images with caption
	preg_match_all('/(\[caption.*\]<img.*\/>.*\[\/caption\])/Ui', $content, $captions);
	if( count( $captions[1] ) > 0 ) {
		// read data attributes and build shortcode
		foreach( $captions[1] as $key => $caption ) {
			$new_caption = do_shortcode( $caption );
			$content = str_replace( $caption, $new_caption, $content );
		}
	}

	preg_match_all('/(<img[\s\w\S][^>]*data-hi_style=[\s\w\S][^>]*)>/Um', $content, $image_array);
...
{
    preg_match_all ('/(data-hi_(style|title|link|view|text|size|share|display|new_window))=("[^"]*")/i', $image, $prop_array);
