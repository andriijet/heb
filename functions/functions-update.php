<?php
	...
	function hi_check_for_updates( $transient )
	{
		...
		if( version_compare( HI_VERSION, $last_version, '<' ) )
		{
			$obj = new stdClass();
			$obj->slug = 'hover-effects-builder';
			$obj->plugin = 'hover-effects-builder/hovers.php';
			$obj->upgrade_notice = __( 'Automatic update is unavailable for this plugin.', 'hitd' ) . ' <a href="' . HI_PLUGIN_CC . '">' . __( 'Download new version from CodeCanyon.', 'hitd' ) . '</a>';
	...
	{
		echo ' <a href="' . HI_PLUGIN_CC . '">' . __( 'Download new version from CodeCanyon.', 'hitd' ) . '</a>';
	}
	...
	function hi_get_remote_information()
	{
		...
		$obj->name = 'Hover Effects Builder';
		$obj->last_updated = date( 'Y/m/d', get_option( HI_UPDATES . '_last' ) );
		$obj->sections = array( 'description' => '<a href="' . HI_PLUGIN_CC . '">' . __( 'Download new version from CodeCanyon.', 'hitd' ) . '</a>', 'changelog' => '' );
		return $obj;
	}

	function hi_versions()
	{
		$xml_file = HI_XML_FILE;
		...
		update_option( HI_UPDATES, $versions );
		update_option( HI_UPDATES . '_last', time() );
