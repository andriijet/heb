...
function hi_assets() {
	...
	// icon font frontend
	wp_enqueue_style( 'hi-icons-frontend', HI_URL . 'assets/css/font-frontend/style.css', false, HI_VERSION );

	// frontend style
	wp_enqueue_style( 'hi-frontend', HI_URL . 'assets/css/hi-frontend.css', false, HI_VERSION );

	// frontend script
	wp_enqueue_script( 'hi-frontend', HI_URL . 'assets/js/hi-frontend.min.js', array( 'jquery' ), HI_VERSION );
}
...
function hi_make( $html, $post_id, $media_id = false ) {
	...
	if ( $slug == false ) {
		...
		// return wrapped html
		return sprintf( '<div class="hi-tpl" data-hidata="%1$sX%2$s">%3$s</div>', $post_id, $media_id, $html );
	}
}
...
function hi_make_hidata( $post_id, $media_id, $api_nick = false ) {
	...
	if ( ! $hi_tpls ) {
		$hi_tpls = hi_get_option( HI_TEMPLATES );
	}

	if ( ! $hi_assigns ) {
		$hi_assigns = hi_get_option( HI_ASSIGNS );
	}
...
function hi_image_shortcode( $atts, $content = null ) {
	...
	$content2 = '<div class="hi-tpl" ';
	$content2 .= 'data-hidata="' . $post->ID . 'X' . $hi_image . '" ';
	if ( $params['alignment'] ) {
		$content2 .= 'data-align="align' . $params['alignment'] . '" ';
	}
	if ( $size_string ) {
		$content2 .= 'data-size="' . $size_string . '" ';
	}
	if ( $params['display'] ) {
		$content2 .= 'data-display="hi-display-' . $params['display'] . '" ';
	}
